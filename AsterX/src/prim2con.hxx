...
CCTK_DEVICE CCTK_HOST void prim2con(const metric &g, const lapse &lap, const shift &shft, const prim &pv, cons &cv) {
  ...
  const array<CCTK_REAL, 3> v_low = calc_vlow(v_up, g.gxx, g.gxy, g.gxz, g.gyy, g.gyz, g.gzz);
      
  const CCTK_REAL w_lorentz = calc_wlor(v_low, v_up);

  /* Computing beta_j */
  const array<CCTK_REAL, 3> beta_up = {shft.betax, shft.betay, shft.betaz};
  const array<CCTK_REAL, 3> beta_low = calc_vlow(beta_up, g.gxx, g.gxy, g.gxz, g.gyy, g.gyz, g.gzz);

  /* Computing B_j */
  const array<CCTK_REAL, 3> B_up = {pv.Bvecx, pv.Bvecy, pv.Bvecz};
  const array<CCTK_REAL, 3> B_low = calc_vlow(B_up, g.gxx, g.gxy, g.gxz, g.gyy, g.gyz, g.gzz);

  /* Computing b^t : this is b^0 * alp */
  const CCTK_REAL bst = w_lorentz * (B_low[0]*v_low[0] + B_low[1]*v_low[1] + B_low[2]*v_low[2]);

  /* Computing b^j */ 
  const CCTK_REAL bsx = (B_up[0] + bst*w_lorentz*(v_up[0] - beta_up[0]/lap.alp)) / w_lorentz;
  const CCTK_REAL bsy = (B_up[1] + bst*w_lorentz*(v_up[1] - beta_up[1]/lap.alp)) / w_lorentz;
  const CCTK_REAL bsz = (B_up[2] + bst*w_lorentz*(v_up[2] - beta_up[2]/lap.alp)) / w_lorentz;
 
  /* Computing b_j */ 
  const array<CCTK_REAL, 3> b_up = {bsx, bsy, bsz};
  array<CCTK_REAL, 3> b_low = calc_vlow(b_up, g.gxx, g.gxy, g.gxz, g.gyy, g.gyz, g.gzz);
  ...
  /* Computing b^mu b_mu */
  const CCTK_REAL bs2 = (B_up[0]*B_low[0] + B_up[1]*B_low[1] + B_up[2]*B_low[2] + bst*bst) / (w_lorentz*w_lorentz);
