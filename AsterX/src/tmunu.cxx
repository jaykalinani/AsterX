  ...
  grid.loop_int_device<0, 0, 0>(
      ...
      [=] CCTK_DEVICE(const PointDesc &p) CCTK_ATTRIBUTE_ALWAYS_INLINE {
        ...
        const vec<CCTK_REAL, 3> vup_avg([&](int i) ARITH_INLINE {
          return calc_avg_c2v(gf_vels(i), p);
        });
	...
	const vec<CCTK_REAL, 3> Bup_avg([&](int i) ARITH_INLINE {
          return calc_avg_c2v(gf_Bvecs(i), p);
        });
  
	const smat<CCTK_REAL, 3> g_low{gxx(p.I), gxy(p.I), gxz(p.I), gyy(p.I), gyz(p.I), gzz(p.I)};
	const vec<CCTK_REAL, 3> beta_up{betax(p.I), betay(p.I), betaz(p.I)};
	
	/* Computing vlow */
	const vec<CCTK_REAL, 3> vlow_avg =
            calc_contraction(g_low, vup_avg);

        /* Computing betalow */
        const vec<CCTK_REAL, 3> beta_low =
            calc_contraction(g_low, beta_up);
	...
	const CCTK_REAL ut_low = -alp(p.I) + calc_contraction(beta_low, vup_avg);
	...
	const CCTK_REAL bst_up = w_lor*(calc_contraction(Bup_avg, vlow_avg))/alp(p.I);
        const vec<CCTK_REAL, 3> bsi_up = (Bup_avg + alp(p.I) * bst_up * w_lor * ui_up)/w_lor;

	/* Computing the lower 4 vector b of the magnetic field */
        const CCTK_REAL bst_low = bst_up * (-alp(p.I)*alp(p.I) + beta_sq) + calc_contraction(bsi_up, beta_low);
        const vec<CCTK_REAL, 3> bsi_low = bst_up * beta_low + calc_contraction(g_low, bsi_up);

	/* Calculating b^2 = b^mu b_mu */
        const CCTK_REAL bs2 = bst_up*bst_low + calc_contraction(bsi_up, bsi_low);

        /* Computing lower T_{\mu\nu} */
        const CCTK_REAL t00 = rhoenthalpyW2 * ut_low*ut_low + press_avg * (-alp(p.I)*alp(p.I) + beta_sq) + (w_lor*w_lor * ut_low*ut_low + 0.5*(-alp(p.I)*alp(p.I) + beta_sq)) * bs2 - bst_low*bst_low;
        const vec<CCTK_REAL, 3> t0 = rhoenthalpyW2 * ut_low*ui_low + press_avg*beta_low + (w_lor*w_lor*ut_low*ui_low + 0.5*beta_low)*bs2 - bst_low*bsi_low;

        const smat<CCTK_REAL, 3> t([&](int i, int j) ARITH_INLINE {
          return rhoenthalpyW2 * ui_low(i)*ui_low(j) + press_avg*g_low(i, j) + (w_lor*w_lor * ui_low(i)*ui_low(j) + 0.5*g_low(i, j))*bs2 - bsi_low(i)*bsi_low(j);
