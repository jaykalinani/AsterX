...
extern "C" void AsterX_SourceTerms(CCTK_ARGUMENTS) {
  ...
  grid.loop_int_device<1, 1, 1>(
      ...
      /* Upper metric */
      const array<CCTK_REAL, 6> ug = calc_upperg(gxx_avg, gxy_avg, gxz_avg, gyy_avg, gyz_avg, gzz_avg, detg);

      /* Computing v_j */
      const array<CCTK_REAL, 3> v_up = {velx(p.I), vely(p.I), velz(p.I)};
      const array<CCTK_REAL, 3> v_low = calc_vlow(v_up, gxx_avg, gxy_avg, gxz_avg, gyy_avg, gyz_avg, gzz_avg);
      
      const CCTK_REAL w_lorentz = calc_wlor(v_low, v_up);
      /* Computing [ \rho(1+\epsilon) + Pgas ]*W^2 */
      const CCTK_REAL rhoenthalpyW2 = ( rho(p.I)*(1.0 + eps(p.I)) + press(p.I) )*
	      w_lorentz*w_lorentz;
      ...
      /* calc_fd2_v2c takes vertex center input, computes edge-center derivatives 
       * along either dir=0, 1 or 2 using 2nd finite difference, i.e, 
       * at the four edges of the cube with p.I as starting point. 
       * The four edge-centered values are then interpolated to the cell-center 
      ...
      /* calc_fd4_v2c takes vertex center input, computes edge-center derivatives 
       * along either dir=0, 1 or 2 using 4th order finite difference/
       * The eight edge-centered values are then interpolated to the center 
       * using 4th order interpolation */
      ...
      /* Computing T_munu */
      const CCTK_REAL t00 = (rhoenthalpyW2 - press(p.I))/ pow2(alp_avg);
      const CCTK_REAL t0x = rhoenthalpyW2*velxshift/alp_avg 
                 	    + press(p.I)*betax_avg/pow2(alp_avg);
      const CCTK_REAL t0y = rhoenthalpyW2*velyshift/alp_avg 
                            + press(p.I)*betay_avg/pow2(alp_avg);
      const CCTK_REAL t0z = rhoenthalpyW2*velzshift/alp_avg 
                            + press(p.I)*betaz_avg/pow2(alp_avg);
      const CCTK_REAL txx = rhoenthalpyW2*velxshift*velxshift 
                            + press(p.I)*(ug[0] - betax_avg*betax_avg/pow2(alp_avg)); //ug[0]=uxx
      const CCTK_REAL txy = rhoenthalpyW2*velxshift*velyshift 
                            + press(p.I)*(ug[1] - betax_avg*betay_avg/pow2(alp_avg)); //ug[1]=uxy
      const CCTK_REAL txz = rhoenthalpyW2*velxshift*velzshift 
                            + press(p.I)*(ug[2] - betax_avg*betaz_avg/pow2(alp_avg)); //ug[2]=uxz
      const CCTK_REAL tyy = rhoenthalpyW2*velyshift*velyshift
                            + press(p.I)*(ug[3] - betay_avg*betay_avg/pow2(alp_avg)); //ug[3]=uyy
      const CCTK_REAL tyz = rhoenthalpyW2*velyshift*velzshift
                            + press(p.I)*(ug[4] - betay_avg*betaz_avg/pow2(alp_avg)); //ug[4]=uyz
      const CCTK_REAL tzz = rhoenthalpyW2*velzshift*velzshift
                            + press(p.I)*(ug[5] - betaz_avg*betaz_avg/pow2(alp_avg)); //ug[5]=uzz

      const CCTK_REAL t0lowx = rhoenthalpyW2*v_low[0]/alp_avg; 
      const CCTK_REAL t0lowy = rhoenthalpyW2*v_low[1]/alp_avg; 
      const CCTK_REAL t0lowz = rhoenthalpyW2*v_low[2]/alp_avg;

      /* Contract the shift with the extrinsic curvature */
      const CCTK_REAL shiftshiftk = betax_avg*betax_avg*kxx_avg
	                            + betay_avg*betay_avg*kyy_avg
			            + betaz_avg*betaz_avg*kzz_avg
		                    + 2.0*(betax_avg*betay_avg*kxy_avg
                                    + betax_avg*betaz_avg*kxz_avg
                                    + betay_avg*betaz_avg*kyz_avg);	    
      const CCTK_REAL shiftkx = betax_avg*kxx_avg + betay_avg*kxy_avg + betaz_avg*kxz_avg;	
      const CCTK_REAL shiftky = betax_avg*kxy_avg + betay_avg*kyy_avg + betaz_avg*kyz_avg;	
      const CCTK_REAL shiftkz = betax_avg*kxz_avg + betay_avg*kyz_avg + betaz_avg*kzz_avg;

      /* Contract the matter terms with the extrinsic curvature */
      const CCTK_REAL sumTK = txx*kxx_avg +tyy*kyy_avg + tzz*kzz_avg
	                      + 2.0*(txy*kxy_avg + txz*kxz_avg + tyz*kyz_avg);

      /* Update term for tau */
      const CCTK_REAL tau_source = t00*(shiftshiftk - (betax_avg*dx_alp 
				   + betay_avg*dy_alp + betaz_avg*dz_alp) )
                                   + t0x*(-dx_alp + 2.0*shiftkx) 
                                   + t0y*(-dy_alp + 2.0*shiftky) 
                                   + t0z*(-dz_alp + 2.0*shiftkz) 
                                   + sumTK;
	
      /* Contract the shift with derivatives of the metric */
      const CCTK_REAL halfshiftdgx = 0.5*(betax_avg*betax_avg*dx_gxx 
                                     + betay_avg*betay_avg*dx_gyy 
				     + betaz_avg*betaz_avg*dx_gzz) 
		                     + betax_avg*betay_avg*dx_gxy 
				     + betax_avg*betay_avg*dx_gxz 
				     + betay_avg*betaz_avg*dx_gyz;

      const CCTK_REAL halfshiftdgy = 0.5*(betax_avg*betax_avg*dy_gxx 
                                     + betay_avg*betay_avg*dy_gyy 
                                     + betaz_avg*betaz_avg*dy_gzz) 
                                     + betax_avg*betay_avg*dy_gxy 
                                     + betax_avg*betay_avg*dy_gxz 
                                     + betay_avg*betaz_avg*dy_gyz;
		
      const CCTK_REAL halfshiftdgz = 0.5*(betax_avg*betax_avg*dz_gxx 
                                     + betay_avg*betay_avg*dz_gyy 
                                     + betaz_avg*betaz_avg*dz_gzz) 
                                     + betax_avg*betay_avg*dz_gxy 
                                     + betax_avg*betay_avg*dz_gxz 
                                     + betay_avg*betaz_avg*dz_gyz;

      /* Contract the matter with derivatives of the metric */

      const CCTK_REAL halfTdgx = 0.5*(txx*dx_gxx + tyy*dx_gyy + tzz*dx_gzz) 
		                 + txy*dx_gxy + txz*dx_gxz + tyz*dx_gyz;
      const CCTK_REAL halfTdgy = 0.5*(txx*dy_gxx + tyy*dy_gyy + tzz*dy_gzz) 
		                 + txy*dy_gxy + txz*dy_gxz + tyz*dy_gyz;
      const CCTK_REAL halfTdgz = 0.5*(txx*dz_gxx + tyy*dz_gyy + tzz*dz_gzz)
                                 + txy*dz_gxy + txz*dz_gxz + tyz*dz_gyz;

      const CCTK_REAL momx_source = t00*(halfshiftdgx - alp_avg*dx_alp) 
      ...
      const CCTK_REAL momy_source = t00*(halfshiftdgy - alp_avg*dy_alp) 
      ...
      const CCTK_REAL momz_source = t00*(halfshiftdgz - alp_avg*dz_alp)
