...
template <int dir> void CalcFlux(CCTK_ARGUMENTS) {
  ...
  const auto calcflux =
      [=] CCTK_DEVICE(array<array<CCTK_REAL, 4>, 2> lam,
                      vec<CCTK_REAL, 2> var, vec<CCTK_REAL, 2> flux)
          CCTK_ATTRIBUTE_ALWAYS_INLINE {
  ...
  grid.loop_int_device<
    ...
    //const vec<vec<CCTK_REAL, 3>, 2> vtilde_rc = alp_avg * vel_c - beta_avg;
    ...
    const vec<CCTK_REAL, 2> dens_h_W_plus_sqrt_detg_W2b2_rc(
        [&](int f) ARITH_INLINE{
        return dens_h_W_rc(f) + sqrt_detg * (pow2(alpha_b0_rc(f)) + B2_rc(f));
        });
    ...
    const vec<CCTK_REAL, 2> tau_rc
      = dens_h_W_rc - dens_rc - sqrt_detg_press_plus_pmag_rc +
            sqrt_detg * B2_rc;

    const vec<vec<CCTK_REAL, 2>, 3>Btildes_rc([&](int i) ARITH_INLINE {
        return sqrt_detg * Bs_rc(i); });

    // Computing fluxes of conserved variables
    const vec<CCTK_REAL, 2> flux_dens([&](int f) ARITH_INLINE {
        return dens_rc(f) * vtilde_rc(f); });
    ...
    fluxBxs[dir](p.I) = (dir != 0) * calcflux(lambda, Btildes_rc(0), flux_Btildes(0));
    fluxBys[dir](p.I) = (dir != 1) * calcflux(lambda, Btildes_rc(1), flux_Btildes(1));
    fluxBzs[dir](p.I) = (dir != 2) * calcflux(lambda, Btildes_rc(2), flux_Btildes(2));
  });
}
