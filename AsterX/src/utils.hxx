...
template <typename T>
CCTK_DEVICE CCTK_HOST array<T, 6> calc_upperg(const T &gxx, const T &gxy, const T &gxz,
...
template <typename T>
CCTK_DEVICE CCTK_HOST T calc_fd2_v2c(const GF3D2<const T> &gf, const PointDesc &p, int dir) {
        constexpr auto DI = PointDesc::DI;
	...
	if(dir==0){
        ...
        dgf4 = (gf(p.I + DI[dir1] + DI[dir2] + DI[dir]) - gf(p.I + DI[dir1] + DI[dir2]))/p.DX[dir];
...
template <typename T>
CCTK_DEVICE CCTK_HOST T calc_fd4_v2c(const GF3D2<const T> &gf, const PointDesc &p, int dir) {
        constexpr auto DI = PointDesc::DI;
        T dgf1, dgf2, dgf3, dgf4;
	...
	dgf8 = ( (1.0/24.0)*gf(p.I - DI[dir1] - DI[dir2] + 2*DI[dir])
        ...
        return (27.0/14.0)*(dgf1 + dgf2 + dgf3 + dgf4) - (10.0/7.0)*(dgf5 + dgf6 + dgf7 + dgf8);
  }

// Computes the components of a lower indice quantity
template <typename T>
CCTK_DEVICE CCTK_HOST const array<T, 3> calc_vlow(const array <T,3> &v_up, const T &gxx, 
		                  const T &gxy, const T &gxz, const T &gyy, const T &gyz, const T &gzz) {
        return {gxx*v_up[0] + gxy*v_up[1] + gxz*v_up[2], //vlowx
...
template <typename T>
CCTK_DEVICE CCTK_HOST const T calc_wlor(const array <T,3> &v_low, const array <T,3> &v_up) {
        return 1.0/ sqrt(1.0 - (v_low[0]*v_up[0] + v_low[1]*v_up[1] + v_low[2]*v_up[2] ));
  }

// Second-order average of vertex-centered grid functions to cell center
template <typename T>
CCTK_DEVICE CCTK_HOST T calc_avg_v2c(const GF3D2<const T> &gf, const PointDesc &p){
     constexpr auto DI = PointDesc::DI;
     T gf_avg = 0.0;
...
// Second-order average of edge-centered grid functions (along dir) to cell center 
template <typename T>
CCTK_DEVICE CCTK_HOST T calc_avg_v2c(const GF3D2<const T> &gf, const PointDesc &p, const int dir){
     constexpr auto DI = PointDesc::DI;
     T gf_avg = 0.0;
