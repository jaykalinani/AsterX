...
$\texttt{IG}_\mathrm{fine} - \texttt{IG}_\mathrm{coarse}$ so both levels
use one gauge integral per shared node. The register never enters the RHS,
changes \texttt{Avec} only within one gauge-stencil width of a refinement
boundary, and is re-zeroed on every full cascade. It is active only with the
generalized Lorenz gauge and \texttt{CarpetX::restrict\_during\_sync = no}
(the mode CarpetX requires under subcycling); otherwise it is inert and the
run is unchanged. Storage for \texttt{IG}, \texttt{IG\_rhs} and the scratch
\texttt{IGr} is always allocated.
