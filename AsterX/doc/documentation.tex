...
function, but no extra kernel and no reconcile. An active run adds, per
time-aligned pass, one nodal restriction and one ghost synchronization of the
scratch ledger \texttt{IGr} plus the local shift and adopt kernels; the
register issues no \texttt{Avec} transfer of its own, since the driver's
restriction and halo refill follow the shift.
...
\texttt{magTOV\_Z4c\_AMR\_SC} runs subcycled with \texttt{gauge\_register = no}
and pins the subcycled path bit-for-bit against reference data that predate
the register;
