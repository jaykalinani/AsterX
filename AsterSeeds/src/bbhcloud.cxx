  ...
  grid.loop_all<1, 1, 1>(
        ...
        [=] CCTK_HOST(const PointDesc &p) CCTK_ATTRIBUTE_ALWAYS_INLINE {
          ...
          case bbh_id_t::Gaussian : {
            ...
            press(p.I) = isentropic ? eos_1p_poly->p_from_valid_gm1(gm1) : press_disk*exp(-pow(p.z,2)/disk_width);
          ...
          case bbh_id_t::Constant : {
            ...
            press(p.I) = isentropic ? eos_1p_poly->p_from_valid_gm1(gm1) : press_disk;
            eps(p.I) = eos_3p_ig->eps_from_valid_rho_press_ye(rho(p.I), press(p.I),
                                                        dummy_ye);
            break;
          };
          case bbh_id_t::PowerLaw : {
            ...
            vely(p.I) = vy_0;
            velz(p.I) = vz_0; 
            press(p.I) = isentropic ? eos_1p_poly->p_from_valid_gm1(gm1) : press_disk * pow(rr + 1e-100, -npress);
            eps(p.I) = eos_3p_ig->eps_from_valid_rho_press_ye(rho(p.I), press(p.I),
                                                        dummy_ye);
            break;
          };
