...
extern "C" void HydroToyCarpetX_Fluxes(CCTK_ARGUMENTS) {
  ...
  griddesc.loop_all_device<1, 1, 1>(
      nghostzones, [=] CCTK_DEVICE CCTK_HOST(
        ...
        if (p1.i == 3 && p1.j == 4 && p1.k == 5) {
          ...
          printf("fxrho_p = %f, fxrho_m =%f\n", (rho[p.idx] * velx[p.idx]),
                 (rho[p.idx - p.di] * velx[p.idx - p.di]));
          printf(
              "fxmomx_p = %f, fxmomx_m =%f\n",
              (momx[p.idx] * velx[p.idx] + press[p.idx],
               momx[p.idx - p.di] * velx[p.idx - p.di] + press[p.idx - p.di]));
          printf(
              "fxmomy_p = %f, fxmomy_m =%f\n",
              (momy[p.idx] * velx[p.idx] + press[p.idx],
               momx[p.idx - p.di] * velx[p.idx - p.di] + press[p.idx - p.di]));
          printf(
              "fxmomz_p = %f, fxmomz_m =%f\n",
              (momz[p.idx] * velx[p.idx] + press[p.idx],
               momx[p.idx - p.di] * velx[p.idx - p.di] + press[p.idx - p.di]));
          printf("fxetot_p = %f, fxetot_m =%f\n",
                 (etot[p.idx] + press[p.idx]) * velx[p.idx],
                 (etot[p.idx - p.di] + press[p.idx - p.di]) *
                     velx[p.idx - p.di]);
        }

        fxrho[p1.idx] =
            calcflux(rho[p.idx], rho[p.idx - p.di], rho[p.idx] * velx[p.idx],
                     rho[p.idx - p.di] * velx[p.idx - p.di]);
        fxmomx[p1.idx] = calcflux(momx[p.idx], momx[p.idx - p.di],
                                  momx[p.idx] * velx[p.idx] + press[p.idx],
                                  momx[p.idx - p.di] * velx[p.idx - p.di] +
                                      press[p.idx - p.di]);
        fxmomy[p1.idx] = calcflux(momy[p.idx], momy[p.idx - p.di],
                                  momy[p.idx] * velx[p.idx] + press[p.idx],
                                  momy[p.idx - p.di] * velx[p.idx - p.di] +
                                      press[p.idx - p.di]);
        fxmomz[p1.idx] = calcflux(momz[p.idx], momz[p.idx - p.di],
                                  momz[p.idx] * velx[p.idx] + press[p.idx],
                                  momz[p.idx - p.di] * velx[p.idx - p.di] +
                                      press[p.idx - p.di]);
        fxetot[p1.idx] = calcflux(etot[p.idx], etot[p.idx - p.di],
                                  (etot[p.idx] + press[p.idx]) * velx[p.idx],
                                  (etot[p.idx - p.di] + press[p.idx - p.di]) *
                                      velx[p.idx - p.di]);
  ...
  griddesc.loop_all_device<1, 1, 1>(
      nghostzones, [=] CCTK_DEVICE CCTK_HOST(
        ...
        if (p1.i == 3 && p1.j == 4 && p1.k == 5) {
          ...
          printf("momz_p = %g, momz_m = %g\n", momz[p.idx], momz[p.idx - p.di]);
          printf("etot_p = %g, etot_m = %g\n", etot[p.idx], etot[p.idx - p.di]);
          printf("fyrho_p = %f, fyrho_m =%f\n", (rho[p.idx] * vely[p.idx]),
                 (rho[p.idx - p.dj] * vely[p.idx - p.dj]));
          printf(
              "fymomx_p = %f, fymomx_m =%f\n",
              (momx[p.idx] * vely[p.idx] + press[p.idx],
               momx[p.idx - p.dj] * vely[p.idx - p.dj] + press[p.idx - p.dj]));
          printf(
              "fymomy_p = %f, fymomy_m =%f\n",
              (momy[p.idx] * vely[p.idx] + press[p.idx],
               momx[p.idx - p.dj] * vely[p.idx - p.dj] + press[p.idx - p.dj]));
          printf(
              "fymomz_p = %f, fymomz_m =%f\n",
              (momz[p.idx] * vely[p.idx] + press[p.idx],
               momx[p.idx - p.dj] * vely[p.idx - p.dj] + press[p.idx - p.dj]));
          printf("fyetot_p = %f, fyetot_m =%f\n",
                 (etot[p.idx] + press[p.idx]) * vely[p.idx],
                 (etot[p.idx - p.dj] + press[p.idx - p.dj]) *
                     vely[p.idx - p.dj]);
        }

        fyrho[p1.idx] =
            calcflux(rho[p.idx], rho[p.idx - p.dj], rho[p.idx] * vely[p.idx],
                     rho[p.idx - p.dj] * vely[p.idx - p.dj]);
        fymomx[p1.idx] = calcflux(momx[p.idx], momx[p.idx - p.dj],
                                  momx[p.idx] * vely[p.idx] + press[p.idx],
                                  momx[p.idx - p.dj] * vely[p.idx - p.dj] +
                                      press[p.idx - p.dj]);
        fymomy[p1.idx] = calcflux(momy[p.idx], momy[p.idx - p.dj],
                                  momy[p.idx] * vely[p.idx] + press[p.idx],
                                  momy[p.idx - p.dj] * vely[p.idx - p.dj] +
                                      press[p.idx - p.dj]);
        fymomz[p1.idx] = calcflux(momz[p.idx], momz[p.idx - p.dj],
                                  momz[p.idx] * vely[p.idx] + press[p.idx],
                                  momz[p.idx - p.dj] * vely[p.idx - p.dj] +
                                      press[p.idx - p.dj]);
        fyetot[p1.idx] = calcflux(etot[p.idx], etot[p.idx - p.dj],
                                  (etot[p.idx] + press[p.idx]) * vely[p.idx],
                                  (etot[p.idx - p.dj] + press[p.idx - p.dj]) *
                                      vely[p.idx - p.dj]);
  ...
  griddesc.loop_all_device<1, 1, 1>(
      nghostzones, [=] CCTK_DEVICE CCTK_HOST(
        ...
        if (p1.i == 3 && p1.j == 4 && p1.k == 5) {
          ...
          printf("fzrho_p = %f, fzrho_m =%f\n", (rho[p.idx] * velz[p.idx]),
                 (rho[p.idx - p.dk] * velz[p.idx - p.dk]));
          printf(
              "fzmomx_p = %f, fzmomx_m =%f\n",
              (momx[p.idx] * velz[p.idx] + press[p.idx],
               momx[p.idx - p.dk] * velz[p.idx - p.dk] + press[p.idx - p.dk]));
          printf(
              "fzmomy_p = %f, fzmomy_m =%f\n",
              (momy[p.idx] * velz[p.idx] + press[p.idx],
               momx[p.idx - p.dk] * velz[p.idx - p.dk] + press[p.idx - p.dk]));
          printf(
              "fzmomz_p = %f, fzmomz_m =%f\n",
              (momz[p.idx] * velz[p.idx] + press[p.idx],
               momx[p.idx - p.dk] * velz[p.idx - p.dk] + press[p.idx - p.dk]));
          printf("fzetot_p = %f, fzetot_m =%f\n",
                 (etot[p.idx] + press[p.idx]) * velz[p.idx],
                 (etot[p.idx - p.dk] + press[p.idx - p.dk]) *
                     velz[p.idx - p.dk]);
        }

        fzrho[p1.idx] =
            calcflux(rho[p.idx], rho[p.idx - p.dk], rho[p.idx] * velz[p.idx],
                     rho[p.idx - p.dk] * velz[p.idx - p.dk]);
        fzmomx[p1.idx] = calcflux(momx[p.idx], momx[p.idx - p.dk],
                                  momx[p.idx] * velz[p.idx] + press[p.idx],
                                  momx[p.idx - p.dk] * velz[p.idx - p.dk] +
                                      press[p.idx - p.dk]);
        fzmomy[p1.idx] = calcflux(momy[p.idx], momy[p.idx - p.dk],
                                  momy[p.idx] * velz[p.idx] + press[p.idx],
                                  momy[p.idx - p.dk] * velz[p.idx - p.dk] +
                                      press[p.idx - p.dk]);
        fzmomz[p1.idx] = calcflux(momz[p.idx], momz[p.idx - p.dk],
                                  momz[p.idx] * velz[p.idx] + press[p.idx],
                                  momz[p.idx - p.dk] * velz[p.idx - p.dk] +
                                      press[p.idx - p.dk]);
        fzetot[p1.idx] = calcflux(etot[p.idx], etot[p.idx - p.dk],
                                  (etot[p.idx] + press[p.idx]) * velz[p.idx],
                                  (etot[p.idx - p.dk] + press[p.idx - p.dk]) *
                                      velz[p.idx - p.dk]);
  ...
  //    }); fzmomy_(p.I) = calcflux(momy_, [&](auto I) { return momy_(I) *
  //    velz_(I); }); fzmomz_(p.I) = calcflux(
