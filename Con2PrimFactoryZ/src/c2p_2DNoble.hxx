...
c2p_1DPalenzuela::get_WLorentz_vsq_bsq_Seeds(
    vec<CCTK_REAL, 3> &B_up, vec<CCTK_REAL, 3> &z_up,
    const smat<CCTK_REAL, 3> &glo) const {
  vec<CCTK_REAL, 3> z_low = calc_contraction(glo, z_up);
  CCTK_REAL zsq = calc_contraction(z_low, z_up);
  CCTK_REAL ZdotB = calc_contraction(z_low, B_up);
  CCTK_REAL ZdotBsq = ZdotB * ZdotB;
  CCTK_REAL Bsq = get_Bsq_Exact(B_up, glo);

  if ((zsq < 0.) && (fabs(zsq) < 1.0e-13)) {
    zsq = fabs(zsq);
  }

  CCTK_REAL w_lor = sqrt(1. + zsq);
  CCTK_REAL bsq = (Bsq+ZdotBsq) / (w_lor * w_lor);
  vec<CCTK_REAL, 3> w_vsq_bsq{w_lor, zsq/w_lor/w_lor, bsq};
...
c2p_2DNoble::WZ2Prim(CCTK_REAL Z_Sol, CCTK_REAL vsq_Sol, CCTK_REAL Bsq,
  ...
  pv.zvec(X) = pv.vel(X)*W_sol;
  pv.zvec(Y) = pv.vel(Y)*W_sol;
  pv.zvec(Z) = pv.vel(Z)*W_sol;
}
...
c2p_2DNoble::solve(EOSType &eos_th, prim_vars &pv, prim_vars &pv_seeds,
  ...
  const vec<CCTK_REAL, 3> w_vsq_bsq = get_WLorentz_vsq_bsq_Seeds(
      pv_seeds.Bvec, pv_seeds.zvec, glo); // this also recomputes pv_seeds.w_lor
