...
public:
  ...
  CCTK_HOST CCTK_ATTRIBUTE_ALWAYS_INLINE inline void init(int init_nrho, FILE* init_in1D, double* init_logrho_arr, const int stencil_size) {
		...
		// Init Rho Array
    if (!(rho_arr = (double*)The_Managed_Arena()->alloc(nrho * sizeof(double)))) {
    CCTK_VError(__LINE__, __FILE__, CCTK_THORNSTRING,
      "Cannot allocate memory for EOS table");
    }
		for(int i=0;i<nrho;i++) rho_arr[i] = exp(init_logrho_arr[i]);
	...
	CCTK_HOST CCTK_ATTRIBUTE_ALWAYS_INLINE inline void read_1dfile__set_array(const int num_header_lines=0) {
		char* line = NULL;
		size_t len = 0;
		ssize_t read;
		int which_line = 0;
    if (!(Ye_rho_arr = (double*)The_Managed_Arena()->alloc(nrho * sizeof(double)))) {
    CCTK_VError(__LINE__, __FILE__, CCTK_THORNSTRING,
      "Cannot allocate memory for EOS table");
    }
		// Skip header line
		while( (read = getline(&line, &len, in1D)) != -1 ) {
			if( which_line >= num_header_lines ) Ye_rho_arr[which_line] = strtod(line, NULL);
			which_line++;
		}
		free(line);
	}

  CCTK_HOST CCTK_DEVICE CCTK_ATTRIBUTE_ALWAYS_INLINE inline void interpolate_1d_quantity_as_function_of_rho(
...
#define MIN(A, B) ( ((A) < (B)) ? (A) : (B) )
	...
	CCTK_HOST CCTK_DEVICE CCTK_ATTRIBUTE_ALWAYS_INLINE inline void linear_interp_1D( const int nx,
