...
extern "C" void ID_TabEOS_HydroQuantities__initial_Y_e(CCTK_ARGUMENTS) {
  ...
  if( (Y_e_file = fopen(Y_e_filename,"r")) == NULL ) {
    ...
    for(int i=0;i<nrho;i++) rho_arr[i] = exp(eos_3p_tab3d.logrho[i]);
		...
		grid.loop_all<0, 0, 0>(
			...
			[=] CCTK_HOST(const PointDesc &p) CCTK_ATTRIBUTE_ALWAYS_INLINE {
				if( rho(p.I) > id_rho_atm_max ) {
					...
					CCTK_REAL Y_eL;
					interpolate_1d_quantity_as_function_of_rho(interp_stencil_size,nrho,rho_arr,Y_e_of_rho_arr,rho(p.I),&Y_eL);
