...
void prim2con(const metric& g, const prim& pv, cons& cv) {
	...
	//determinant of spatial metric
          const CCTK_REAL detg = -g.gxz*g.gxz*g.gyy + 2.0*g.gxy*g.gxz*g.gyz - g.gxx*g.gyz*g.gyz
                                 - g.gxy*g.gxy*g.gzz + g.gxx*g.gyy*g.gzz;
          ...
          //w_lorentz
          const CCTK_REAL w_lorentz = 1.0 / sqrt(1 - (g.gxx*pv.velx*pv.velx + g.gyy*pv.vely*pv.vely 
          ...
          cv.tau = sqrt_detg * pv.rho * w_lorentz * ( (1 + pv.eps
                                + pv.press/pv.rho) * w_lorentz - 1) - pv.press;
