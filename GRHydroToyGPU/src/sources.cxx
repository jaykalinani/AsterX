...
template <int dir> void CalcFlux(CCTK_ARGUMENTS) {
  ...
  */
  ...
  grid.loop_int_device<
      1, 1, 1>(grid.nghostzones, [=] CCTK_DEVICE CCTK_HOST(
                                     const PointDesc
    ...
    const int nvars = 10;
    const CCTK_REAL *const restrict vars[nvars] = {
        beta1, beta2, beta3, alp, g11, g12, g13, g22, g23, g33};
    ...
    const CCTK_REAL vlowx =
        g11[ijk] * velx[ijk] + g12[ijk] * vely[ijk] + g13[ijk] * velz[ijk];
    const CCTK_REAL vlowy =
        g12[ijk] * velx[ijk] + g22[ijk] * vely[ijk] + g23[ijk] * velz[ijk];
    const CCTK_REAL vlowz =
        g13[ijk] * velx[ijk] + g23[ijk] * vely[ijk] + g33[ijk] * velz[ijk];
    const CCTK_REAL Bvecxlow = do_mhd ? g11[ijk] * Bvecx[ijk] +
                                            g12[ijk] * Bvecy[ijk] +
                                            g13[ijk] * Bvecz[ijk]
                                      : 0;
    const CCTK_REAL Bvecylow = do_mhd ? g12[ijk] * Bvecx[ijk] +
                                            g22[ijk] * Bvecy[ijk] +
                                            g23[ijk] * Bvecz[ijk]
                                      : 0;
    const CCTK_REAL Bveczlow = do_mhd ? g13[ijk] * Bvecx[ijk] +
                                            g23[ijk] * Bvecy[ijk] +
                                            g33[ijk] * Bvecz[ijk]
                                      : 0;

    //  B^i v_i (= b^0/u^0)
    const CCTK_REAL Bdotv =
        do_mhd ? vlowx * Bvecx[ijk] + vlowy * Bvecy[ijk] + vlowz * Bvecz[ijk]
               : 0;

    // v^2 = v_i v^i; w=(1-v^2)^{-1/2}

    const CCTK_REAL v2 =
        vlowx * velx[ijk] + vlowy * vely[ijk] + vlowz * velz[ijk];
    const CCTK_REAL invw = sqrt(1.0 - v2);
    const CCTK_REAL w = 1. / invw;

    // b^2 = B^i B_i / w^2 + (b^0/u^0)^2

    const CCTK_REAL b2 = do_mhd
                             ? (Bvecx[ijk] * Bvecxlow + Bvecy[ijk] * Bvecylow +
                                Bvecz[ijk] * Bveczlow) *
                                       SQR(invw) +
                                   SQR(Bdotv)
                             : 0;
    ...
    // conservatives
    const CCTK_REAL rhohstarW2 =
        (rho[ijk] * (one + eps[ijk]) + (press[ijk] + b2)) * SQR(w);
    ...
    const CCTK_REAL t0x =
        rhohstarW2 * velxshift * invalp + pstar * shiftx * invalp2 - bt * bx;
    const CCTK_REAL t0y =
        rhohstarW2 * velyshift * invalp + pstar * shifty * invalp2 - bt * by;
    const CCTK_REAL t0z =
        rhohstarW2 * velzshift * invalp + pstar * shiftz * invalp2 - bt * bz;
    ...
    const CCTK_REAL shiftshiftk =
        shiftx * shiftx * k11[ijk] + shifty * shifty * k22[ijk] +
        shiftz * shiftz * k33[ijk] +
        two * (shiftx * shifty * k12[ijk] + shiftx * shiftz * k13[ijk] +
               shifty * shiftz * k23[ijk]);

    const CCTK_REAL shiftkx =
        shiftx * k11[ijk] + shifty * k12[ijk] + shiftz * k13[ijk];
    const CCTK_REAL shiftky =
        shiftx * k12[ijk] + shifty * k22[ijk] + shiftz * k23[ijk];
    const CCTK_REAL shiftkz =
        shiftx * k13[ijk] + shifty * k23[ijk] + shiftz * k33[ijk];

    //        Contract the matter terms with the extrinsic curvature

    const CCTK_REAL sumTK =
        txx * k11[ijk] + tyy * k22[ijk] + tzz * k33[ijk] +
    ...
    const CCTK_REAL tau_source =
        t00 * (shiftshiftk -
               (shiftx * dx_alp + shifty * dy_alp + shiftz * dz_alp)) +
        t0x * (-dx_alp + two * shiftkx) + t0y * (-dy_alp + two * shiftky) +
        t0z * (-dz_alp + two * shiftkz) + sumTK;
    ...
    const CCTK_REAL halfshiftdgx =
        half * (shiftx * shiftx * dx_gxx + shifty * shifty * dx_gyy +
                shiftz * shiftz * dx_gzz) +
        shiftx * shifty * dx_gxy + shiftx * shiftz * dx_gxz +
        shifty * shiftz * dx_gyz;
    const CCTK_REAL halfshiftdgy =
        half * (shiftx * shiftx * dy_gxx + shifty * shifty * dy_gyy +
                shiftz * shiftz * dy_gzz) +
        shiftx * shifty * dy_gxy + shiftx * shiftz * dy_gxz +
        shifty * shiftz * dy_gyz;
    const CCTK_REAL halfshiftdgz =
        half * (shiftx * shiftx * dz_gxx + shifty * shifty * dz_gyy +
                shiftz * shiftz * dz_gzz) +
    ...
    const CCTK_REAL halfTdgx =
        half * (txx * dx_gxx + tyy * dx_gyy + tzz * dx_gzz) + txy * dx_gxy +
        txz * dx_gxz + tyz * dx_gyz;
    const CCTK_REAL halfTdgy =
        half * (txx * dy_gxx + tyy * dy_gyy + tzz * dy_gzz) + txy * dy_gxy +
        txz * dy_gxz + tyz * dy_gyz;
    const CCTK_REAL halfTdgz =
        half * (txx * dz_gxx + tyy * dz_gyy + tzz * dz_gzz) + txy * dz_gxy +
        txz * dz_gxz + tyz * dz_gyz;

    const CCTK_REAL sx_source =
        t00 * (halfshiftdgx - alp[ijk] * dx_alp) + halfTdgx +
    ...
    const CCTK_REAL sy_source =
        t00 * (halfshiftdgy - alp[ijk] * dy_alp) + halfTdgy +
    ...
    const CCTK_REAL sz_source =
        t00 * (halfshiftdgz - alp[ijk] * dz_alp) + halfTdgz +
    ...
    if (do_Avec) {
      ...
      const CCTK_REAL Avcx_source =
          alp[ijk] * sqrtdet *
          (velyshift * Bvecz[ijk] - velzshift * Bvecy[ijk]);
      const CCTK_REAL Avcy_source =
          alp[ijk] * sqrtdet *
          (velzshift * Bvecx[ijk] - velxshift * Bvecz[ijk]);
      const CCTK_REAL Avcz_source =
          alp[ijk] * sqrtdet *
          (velxshift * Bvecy[ijk] - velyshift * Bvecx[ijk]);
    ...
    if (do_clean_divergence) {
      ...
      const CCTK_REAL dx_det_bydet =
          uxx * dx_gxx + uyy * dx_gyy + uzz * dx_gzz +
          two * (uxy * dx_gxy + uxz * dx_gxz + uyz * dx_gyz);
      const CCTK_REAL dy_det_bydet =
          uxx * dy_gxx + uyy * dy_gyy + uzz * dy_gzz +
          two * (uxy * dy_gxy + uxz * dy_gxz + uyz * dy_gyz);
      const CCTK_REAL dz_det_bydet =
          uxx * dz_gxx + uyy * dz_gyy + uzz * dz_gzz +
      ...
      psidcrhs[ijk] =
          -one * (kap_dc * alp[ijk] + dx_betax + dy_betay + dz_betaz) *
              psidc[ijk] +
          Bconsx[ijk] *
              (dx_alp - half * alp[ijk] *
                            (uxx * dx_gxx + uyy * dx_gyy + uzz * dx_gzz +
                             two * uxy * dx_gxy + two * uxz * dx_gxz +
                             two * uyz * dx_gyz)) *
              invsqrtdet +
          Bconsy[ijk] *
              (dy_alp - half * alp[ijk] *
                            (uxx * dy_gxx + uyy * dy_gyy + uzz * dy_gzz +
                             two * uxy * dy_gxy + two * uxz * dy_gxz +
                             two * uyz * dy_gyz)) *
              invsqrtdet +
          Bconsz[ijk] *
              (dz_alp - half * alp[ijk] *
                            (uxx * dz_gxx + uyy * dz_gyy + uzz * dz_gzz +
                             two * uxy * dz_gxy + two * uxz * dz_gxz +
                             two * uyz * dz_gyz)) *
              invsqrtdet;

      bvcx_source =
          -one * (Bconsx[ijk] * dx_betax + Bconsy[ijk] * dy_betax +
                  Bconsz[ijk] * dz_betax) +
          psidc[ijk] * sqrtdet *
              ((uxx * dx_alp + uxy * dy_alp + uxz * dz_alp) +
               alp[ijk] * (half * (uxx * dx_det_bydet + uxy * dy_det_bydet +
                                   uxz * dz_det_bydet) -
                           (uxx * gdg_x + uxy * gdg_y + uxz * gdg_z)));

      bvcy_source =
          -one * (Bconsx[ijk] * dx_betay + Bconsy[ijk] * dy_betay +
                  Bconsz[ijk] * dz_betay) +
          psidc[ijk] * sqrtdet *
              ((uxy * dx_alp + uyy * dy_alp + uyz * dz_alp) +
               alp[ijk] * (half * (uxy * dx_det_bydet + uyy * dy_det_bydet +
                                   uyz * dz_det_bydet) -
                           (uxy * gdg_x + uyy * gdg_y + uyz * gdg_z)));

      bvcz_source =
          -one * (Bconsx[ijk] * dx_betaz + Bconsy[ijk] * dy_betaz +
                  Bconsz[ijk] * dz_betaz) +
          psidc[ijk] * sqrtdet *
              ((uxz * dx_alp + uyz * dy_alp + uzz * dz_alp) +
               alp[ijk] * (half * (uxz * dx_det_bydet + uyz * dy_det_bydet +
                                   uzz * dz_det_bydet) -
    ...
    array<CCTK_REAL, 2> dens_r = {sqrt_detg_r * rho_r[0] * w_lorentz_r[0],
    ...
    gf_fluxdens(p.I) = calcflux(dens_r[0], dens_r[1], flux_dens_m, flux_dens_p);
