...
private:
  ...
  struct recursive_interpolate_single<F_t, 0, var, CI_t, X1> {
    ...
    linterp(F_t const &F, std::array<size_t, dim> const &index, X1 const &x1) {
      ...
      auto const A = F[var + num_vars * compressed_indexr_t<CI_t, 0>::value(index, F.num_points)];
      auto const B = F[var + num_vars * compressed_indexr_t<CI_t, 1>::value(index, F.num_points)];
...
public:
  ...
  CCTK_HOST
    ...
    static_assert(
        ...
        "Interpolation points must be compatible with the data type of the arrays.");
    static_assert(
        sizeof...(Xt) == dim,
        "You need to provide the correct number of interpolation points");
    static_assert(sizeof...(vars) <= num_vars,
                  "You are trying to interpolate more variables than this container holds.");

    CCTK_INT dir = -1;
    auto const index = std::array<size_t, dim>{(++dir, find_index(dir, xin))...};
