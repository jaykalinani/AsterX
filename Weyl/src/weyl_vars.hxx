...
template <typename T> struct weyl_vars_metric {
  ...
  inline ARITH_INLINE ARITH_DEVICE ARITH_HOST weyl_vars_metric(
      const smat<T, 3> &gamma, const T &alpha, const vec<T, 3> &beta,
      //
      const smat<T, 3> &K, const T &dtalpha, const vec<T, 3> &dtbeta,
      //
      const smat<vec<T, 3>, 3> &dgamma, const vec<T, 3> &dalpha,
      const vec<vec<T, 3>, 3> &dbeta,
      //
      const smat<T, 3> &dtK, const T &dt2alpha, const vec<T, 3> &dt2beta,
      //
      const smat<vec<T, 3>, 3> &dK, const vec<T, 3> &ddtalpha,
      const vec<vec<T, 3>, 3> &ddtbeta,
      //
      const smat<smat<T, 3>, 3> &ddgamma, const smat<T, 3> &ddalpha,
      const vec<smat<T, 3>, 3> &ddbeta)
      : gamma(gamma), alpha(alpha), beta(beta),
        ...
        ddbetal([&](int a) ARITH_INLINE {
          return smat<T, 3>([&](int b, int c) ARITH_INLINE {
            return sum<3>([&](int x) ARITH_INLINE {
        ...
        ddg([&](int a, int b) ARITH_INLINE {
          return smat<T, 4>([&](int c, int d) ARITH_INLINE {
            if (c == 0 && d == 0) {
              if (a == 0 && b == 0)
                return dt2gtt;
              if (a == 0)
...
template <typename T> struct weyl_vars_curvature {
  ...
  // Derivative of inverse of 4-metric
  ...
  weyl_vars_curvature(const smat<T, 4> &g, const smat<vec<T, 4>, 4> &dg,
                      const smat<smat<T, 4>, 4> &ddg)
      : g(g), dg(dg), ddg(ddg),
        //
        detg(min(1.0e+6, max(1.0e-6, calc_det(g)))), //
        gu(calc_inv(g, detg)),                       //
        //
        dgu(calc_dgu(gu, dg)),
        //
...
template <typename T> struct weyl_vars_scalars {
  ...
  inline ARITH_INLINE ARITH_DEVICE ARITH_HOST
  weyl_vars_scalars(const vec<T, 4> &coord, const smat<T, 4> &g,
                    const smat<T, 4> &R, const rten<T, 4> &C)
      : coord(coord), g(g), R(R), C(C),
        //
        detg(calc_det(g)),     //
        gu(calc_inv(g, detg)), //
        //
