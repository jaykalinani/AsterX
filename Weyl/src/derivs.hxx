...
inline T deriv(const GF3D1<const T> &gf_, const vect<int, dim> &I,
               const vec3<T, UP> &dx) {
...
inline enable_if_t<(dir1 == dir2), T> deriv2(const GF3D1<const T> &gf_,
...
template <typename T>
inline vec3<T, DN> deriv(const GF3D1<const T> &gf_, const vect<int, dim> &I,
                         const vec3<T, UP> &dx) {
  return {
      deriv<0>(gf_, I, dx),
      deriv<1>(gf_, I, dx),
...
template <typename T>
inline mat3<T, DN, DN> deriv2(const GF3D1<const T> &gf_,
